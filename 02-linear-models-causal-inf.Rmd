...
Using our model, we can again use `r downlit::autolink("tidybayes::add_predicted_draws()")` to get the model based predictions to fill in the table.
...
To make the priors easier, we will standardize our variables. Using standardized variables, we can easily set priors that encompass the range of scientifically plausible regression lines. For the intercept, we'll use a $\text{Normal(0, 0.2)}$ prior, and for the slope we'll use a $\text{Normal(0, 0.5)}$ prior.
...
tibble(group = seq_len(n),
       alpha = rnorm(n, 0, 0.2),
       beta = rnorm(n, 0, 0.5)) %>%
  expand(nesting(group, alpha, beta), age_s = seq(-3, 3, by = 1)) %>%
  mutate(weight = alpha + beta * age_s) %>%
  ggplot(aes(x = age_s, y = weight, group = group)) +
  geom_line() +
  expand_limits(x = c(-3, 3), y = c(-3, 3)) +
  scale_x_continuous(breaks = seq(-5, 5, 1)) +
  scale_y_continuous(breaks = seq(-5, 5, 1)) +
  labs(x = "Age (standardized)", y = "Weight (standardized)")
```
...
kid_dat <- Howell1 %>%
  filter(age < 13) %>%
  mutate(age_s = standardize(age),
         weight_s = standardize(weight))

w2h2 <- brm(weight_s ~ 1 + age_s, data = kid_dat, family = gaussian,
...
Visualizing our posterior, we see that there is a strong causal relationship between age and weight (`b_age_s`; assuming our DAG is correct). The posterior distribution for `b_age_s` is entirely well above 0.

```{r w2h2-3}
gather_draws(w2h2, b_Intercept, b_age_s, sigma) %>% 
...
We can create separate regression lines for each sex by sex (as a factor) to the model formula. Because we're now estimating an intercept for each sex, we use the `~ 0` code to indicate that we are not estimating a global intercept, and are treating the `sex` variable as an index variable.
...
w2h3 <- brm(weight_s ~ 0 + age_s + sex, data = kid_dat, family = gaussian,
...
Looking at the posterior samples, we see that the slope, `b_age_s`, is very similar to the previous question. This makes since, because we are not estimating separate slopes for each sex. Rather, we estimated separate intercepts, resulting in two parallel lines. When comparing the two intercepts, we see that the posterior distribution for women (`b_sex1`) is lower on average than the distribution for mean (`b_sex2`).

```{r w2h3-2}
gather_draws(w2h3, b_age_s, b_sex1, b_sex2, sigma) %>% 
  ggplot(aes(x = .value, y = .variable)) +
  stat_halfeye() +
  labs(x = "Parameter value", y = "Parameter")
```

However, we are interested in the mean difference between men and women, not the difference in means. To estimate the contrast we need to calculate posterior simulations for each sex. This represents the distribution of expected weights for individuals of each sex, and is shown in the left panel below. Even though the intercepts are fairly distinct, the predictive distributions are quite similar. Finally, we can calculate the difference between the two predictive distributions. The posterior distribution of the difference is shown in the panel on the right, and shows that the difference is very small and centered right at zero. Thus, would conclude that there is no difference in the predicted weight for boys and girls when conditioning on height.

<details><summary>Code to reproduce</summary>

```{r w2h3-3, fig.show = "hide", fig.asp = 0.5}
library(geomtextpath)
library(patchwork)

post <- as_draws_df(w2h3) %>% 
  mutate(female = rnorm(n(),
                        mean = b_sex1 + rnorm(n(), 0, 1) * b_age_s,
                        sd = sigma),
         male = rnorm(n(),
                      mean = b_sex2 + rnorm(n(), 0, 1) * b_age_s,
                      sd = sigma),
         diff = male - female)

ggplot(post) +
  geom_density(aes(x = diff)) -> p
pg <- ggplot_build(p)

ggplot(post) +
  geom_textdensity(aes(x = female), label = "Women",
                   hjust = 0.2, color = "#56B4E9", linewidth = 1, size = 4) +
  geom_textdensity(aes(x = male), label = "Men",
                   hjust = 0.3, color = "#E69F00", linewidth = 1, size = 4) +
  labs(x = "Posterior predicted weight (kg, standardized)", y = "Density") -> p1

ggplot(pg$data[[1]]) +
  geom_area(data = ~filter(.x, x < 0), aes(x = x, y = y),
            fill = "#56B4E9") +
  geom_area(data = ~filter(.x, x >= 0), aes(x = x, y = y),
            fill = "#E69F00") +
  labs(x = "Posterior weight contrast (kg, standardized)", y = "Density") -> p2
...
```{r w2h3-4, echo = FALSE, out.width = "90%"}
knitr::include_graphics(knitr::fig_chunk("w2h3-3", "png"))
```
