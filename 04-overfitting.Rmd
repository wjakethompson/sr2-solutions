...
In words, this means take the log of the average probability across samples of each observation $i$ and sum them together. Thus, a larger sample size will necessarily lead to a larger log-pointwise-predictive-density, even if the data generating process and models are exactly equivalent. More observations are entered into the sum, leading to a larger final lppd, which will in turn increase the information criteria. We can run a quick simulation to demonstrate. For three different sample sizes, we'll simulate 100 data sets, estiamte a linear model, and then calculate the LPPD, WAIC, and PSIS for each.
...
Now, we can visualize the distribution
...
ggplot(aes(x = value)) +
  facet_grid(rows = vars(sample_size), cols = vars(name)) +
