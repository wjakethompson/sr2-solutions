...
```{r e9h6-4, include = FALSE}
qi <- mean_qi(c(chain1, chain2, chain3), .width = 0.95)
```

The last step is to actually look at our estimate of the proportion of water. There's an expected value of `r round(qi$y, 2)`, with a 95% interval of `r round(qi$ymin, 2)` to `r round(qi$ymax, 2)`.
...
```{r e9h7-5, include = FALSE}
qi <- mean_qi(c(chain1, chain2, chain3), .width = 0.95, na.rm = TRUE)
```

Looking at our posterior, we ahve an expected value of `r round(qi$y, 2)`, with a 95% interval of `r round(qi$ymin, 2)` to `r round(qi$ymax, 2)`. This is very similar to what we achieved with the Metropolis algorithm.
