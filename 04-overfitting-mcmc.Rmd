...
To examine the posteriors, we'll first look at the correlations between parameters. In both models, there is a very strong negative correlation between $\beta_L$ and $\beta_R$. This makes sense given our data. Because the data are so strongly correlated, that they provide basically the same information. So if we take more information from the left leg (i.e., higher values of $\beta_L$), we necessarily must let less from teh right leg. This results in in the negative correlation.
