...
```{r e11h2-1, message = FALSE, warning = FALSE}
...
```{r e11h4-1, out.width = "40%", echo = FALSE}
library(ggdag)
...
```{r e11h5-1, out.width = "40%", echo = FALSE}
library(ggdag)

dag_coords <- tibble(name = c("G", "D", "A", "S"),
                     x = c(1, 2, 3, 3),
                     y = c(1, 2, 1, 2))
...
dagify(A ~ G + D + S,
       ...
       coords = dag_coords) %>% 
  ggplot(aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_point(data = . %>% filter(name %in% c("S")),
...
```{r e11h6-5, out.width = "40%", echo = FALSE}
library(ggdag)

dag_coords <- tibble(name = c("B", "E", "S"),
                     x = c(1, 2, 3),
                     y = c(1, 2, 1))
...
```{r w5h4-1, out.width = "40%", echo = FALSE}
library(ggdag)
