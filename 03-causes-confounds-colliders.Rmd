...
The question specifically asks for the counterfactual effect of halving a state's marriage rate. This is dependent on what the original marriage rate was. For this problem, we'll use the average marriage rate across all states, which is about 20. Half of that is 10, which in our standardized units that were used to fit the model is `r round((10 - mean(WaffleDivorce$Marriage)) / sd(WaffleDivorce$Marriage), 2)`. Looking at the counterfactual plot, moving from an $M$ of 0 to an $M$ of -2.66 would result in a decrease in the divorce rate of about 1 standard deviation. We can calculate this directly using code as well. Here we also see an estimated difference of -1.03 standard deviations, with an 89% posterior interval of -3.14 to 1.08.
...
```{r e6e3-1, echo = FALSE, out.width = "25%"}
...
```{r e6e3-2, echo = FALSE, out.width = "25%"}
...
```{r e6e3-3, echo = FALSE, out.width = "25%"}
...
```{r e6e3-4, echo = FALSE, out.width = "25%"}
...
```{r e6m2-3, out.width = "40%", echo = FALSE, fig.cap = " "}
...
```{r e6h-fox, out.width = "40%", echo = FALSE, wrap = "table-question"}
...
```{r w3h1-inst, out.width = "40%", echo = FALSE, wrap = "table-question"}
...
ggsave("w3h1-fox.png", plot = p, path = here("img"), width = (700 / 96),
       height = (700 / 96) * (1 / 1.618), 
       units = "in", dpi = 288, bg = "transparent")

knitr::include_graphics(here("img", "w3h1-fox.png"))
```
...
```{r w3h3-inst, out.width = "40%", echo = FALSE, wrap = "table-question"}
...
coordinates(stroke_dag) <- list(
  x = c(A = 1, S = 1, X = 2, U = 2.5, Y = 3),
  y = c(A = 1, S = 3, X = 2, U = 3,   Y = 2)
)

p <- stroke_dag %>%
  tidy_dagitty() %>%
  ggplot(aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_node(data = ~filter(.x, name == "U"), color = "black", shape = 21) +
  geom_dag_text(color = "black", size = 10) +
  geom_dag_edges(edge_color = "black", edge_width = 2,
                 arrow_directed = grid::arrow(length = grid::unit(15, "pt"),
                                              type = "closed")) +
...
ggsave("w3h3-stroke.png", plot = p, path = here("img"), width = (700 / 96),
       height = (700 / 96) * (1 / 1.618), 
       units = "in", dpi = 288, bg = "transparent")

knitr::include_graphics(here("img", "w3h3-stroke.png"))
```
...
However, the unobserved variable $U$ has important implications for the interpretation of the model. There are three slope coefficients that would be in the canonical Table 2: $\beta_X$, $\beta_S$, and $\beta_A$.

We'll start with $X$. From the perspective of $X$, our DAG looks like this, conditional on $A$ and $S$. Thus, $\beta_X$ represents the direct (and total as there are no indirect paths) causal effect of $X$ on $Y$.

```{r w3h3-2, out.width = "40%", echo = FALSE}
x_dag <- dagitty("dag { X -> Y <- A -> S -> Y <- U -> S }")
coordinates(x_dag) <- list(
  x = c(A = 1, S = 1, X = 2, U = 2.5, Y = 3),
  y = c(A = 1, S = 3, X = 2, U = 3,   Y = 2)
)
...
Next is $S$. Here is our updated DAG, with paths that are eliminated by conditioning on $X$ and $A$. Here, $\beta_S$ is confounded. It is not the total causal effect, because the path of $S$ through $X$ has been blocked. However, $\beta_S$ is also not the direct effect of $S$ on $Y$ because the backdoor path through $U$ is still open. Thus, $\beta_S$ is only a partial causal effect.

```{r w3h3-3, out.width = "40%", echo = FALSE}
s_dag <- dagitty("dag { U -> S -> Y <- X <- A -> Y <- U }")
coordinates(s_dag) <- list(
  x = c(A = 1, S = 1, X = 2, U = 2.5, Y = 3),
  y = c(A = 1, S = 3, X = 2, U = 3,   Y = 2)
)
...
Finally, we'll look at $A$. In the full DAG, the path from $A \rightarrow S \leftarrow U -> Y$ is closed because $S$ is a collider. However, we have opened that path by conditioning on $S$. Thus, from the perspective of $A$, our DAG looks like the image below. We can see that $A$ is hopelessly confounded. Not all causal paths are included, and biased paths have been included (i.e., the collider). Interpret $\beta_A$ at your own risk.

```{r w3h3-4, out.width = "40%", echo = FALSE}
a_dag <- dagitty("dag { S -> X -> Y <- A -> S <- U -> Y <- S }")
coordinates(a_dag) <- list(
  x = c(A = 1, S = 1, X = 2, U = 2.5, Y = 3),
  y = c(A = 1, S = 3, X = 2, U = 3,   Y = 2)
)
